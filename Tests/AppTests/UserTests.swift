...
final class UserTests: XCTestCase {
  ...
  func testUsersCanBeRetrievedFromAPI() throws {
    ...
    XCTAssertEqual(users.count, 3, "User count should be 3")
    XCTAssertEqual(users[1].name, usersName, "User name should be Alice")
    XCTAssertEqual(users[1].username, usersUsername, "User username should be alice")
    XCTAssertEqual(users[1].id, user.id, "User ID should be \(user.id!)")
  }
  
  func testUserCanBeSavedWithAPI() throws {
    let user = User(name: usersName, username: usersUsername, password: "password")
    let receivedUser = try app.getResponse(
        to: usersURI, method: .POST,
        headers: ["Content-Type": "application/json"],
        data: user, decodeTo: User.Public.self, loggedInRequest: true)
