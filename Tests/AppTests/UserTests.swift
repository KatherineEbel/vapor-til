...
final class UserTests: XCTestCase {
  ...
  func testUsersCanBeRetrievedFromAPI() throws {
    ...
    XCTAssertEqual(users.count, 2, "User count should be 2")
    XCTAssertEqual(users[0].name, usersName, "User name should be Alice")
    XCTAssertEqual(users[0].id, user.id, "User ID should be \(user.id!)")
  }
