...
struct WebsiteController: RouteCollection {
  ...
  func indexHandler(_ req: Request) throws -> Future<View> {
    return Acronym.query(on: req).all().flatMap(to: View.self, { acronyms in
      ...
      let showCookieMessage = req.http.cookies["cookies-accepcted"] == nil
  ...
  func userHandler(_ req: Request) throws -> Future<View> {
    return try req.parameters.next(User.self).flatMap(to: View.self) { user in
      return try user.acronyms
          .query(on: req)
          .all().flatMap(to: View.self) { acronyms in
  ...
  func createAcronymPostHandler(_ req: Request, data: CreateAcronymData) throws -> Future<Response> {
    ...
    return acronym.save(on: req)
      .flatMap(to: Response.self) { acronym in
        ...
        let categorySaves: [Future<Void>] = try (data.categories ?? []).compactMap({ category -> Future<Void> in
          return try Category.addCategory(category, to: acronym, on: req)
        })
        ...
        .flatMap(to: Response.self) { savedAcronym in
          ...
          return try acronym
              ...
              .flatMap(to: Response.self) { existingCategories in
                ...
                var categoryResults = try categoriesToAdd.compactMap { newCategory -> Future<Void>? in
                  return try Category.addCategory(newCategory, to: acronym, on: req)
                }
  ...
  func loginPostHandler(_ req: Request, userData: LoginPostData) throws -> Future<Response> {
    return User.authenticate(
        ...
        using: BCryptDigest(), on: req).map(to: Response.self) {
      user in
